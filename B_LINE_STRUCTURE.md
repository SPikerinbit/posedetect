...
```mermaid
flowchart TD
    A[Input Pose Sequence\nT x 33 x C] --> B[Preprocess + Flatten XY\nT x 66]
    B --> C[Resample/Padding to seq_len=256\n256 x 66]
    C --> D[Linear Projection\n66 -> 256 + ReLU + Dropout]
    D --> E1[Temporal Conv Block d=1\n2x Conv1d(k=3)+BN+ReLU + residual]
    E1 --> E2[Temporal Conv Block d=2\n2x Conv1d(k=3)+BN+ReLU + residual]
    E2 --> E3[Temporal Conv Block d=4\n2x Conv1d(k=3)+BN+ReLU + residual]
    E3 --> F[Transformer Encoder x2\n8 heads]
    F --> G1[Cls Head\nLinear 256->3]
    F --> G2[Reg Head\nLinear 256->2 + Softplus]
    G1 --> H[Per-frame Class Probabilities]
    G2 --> I[Per-frame L/R Boundary Distances]
    H --> J[Decode: threshold + boundary expansion]
    I --> J
